...
test "en passant checking" {
    ...
    var tmp: Board_state = try chessl.getBoardFromFen(arena, "5bnr/5ppp/1Q6/2Bkp3/3pP3/3P4/5PPP/4KBNR b H e3 0 39");
...
test "perft - startpos" {
    ...
    var board: Board_state = try chessl.getBoardFromFen(arena, chessl.DEFAULT_FEN);
    var threaded: std.Io.Threaded = .init_single_threaded;
    const io = threaded.io();
    //try std.testing.expect(!hashl.isHashTable_init());
    for (1..perft_MAX_DEPTH + 1) |depth| {
        const _start = std.Io.Timestamp.now(io, .real).toMicroseconds();
        ...
        const timeTaken = std.Io.Timestamp.now(io, .real).toMicroseconds() - _start;
        try std.testing.expectEqual(expect, ExpectedBenchmarkResults[depth]);
...
test "draw detection" {
    ...
    const arena = arena_allocator.allocator();
    mainl.initAll(arena, false);
    var tmp: Board_state = try chessl.getBoardFromFen(arena, chessl.DEFAULT_FEN);
    try chessl.applyUciMoves(&tmp, "position startpos e2e3 e7e5 d1f3 c7c6 d2d4 d8e7 c1d2 e5d4 f1d3 e7e5 f3g3 e5g3 h2g3 d4e3 d2e3 f8b4 e3d2 b4d2 b1d2 g8f6 d2c4 d7d5 c4d6 e8e7 d6c8 h8c8 f2f4 e7e6 g1f3 c6c5 f3g5 e6e7 c2c3 h7h6 g5f3 b8c6 d3b5 a7a6 b5a4 c6a5 f3e5 e7e6 g3g4 a5c4 e5c4 d5c4 e1c1 e6e7 g4g5 h6g5 f4g5 f6g4 d1d7 e7e6 d7b7 c8d8 h1f1 d8d6 f1f7 e6d5 a4d1 g4e5 d1f3 e5f3 g2f3 g7g6 b7e7 a8h8 f7h7 h8h7 e7h7 d5e5 h7f7 d6b6 c1b1 b6c6 b1c1 c6b6 c1b1 b6c6 b1c1", arena, false);
    const allMoves = moveGenl.generateLegalMoves(&tmp);
